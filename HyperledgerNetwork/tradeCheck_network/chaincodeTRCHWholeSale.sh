...
#Chaincode Install
docker exec -e "CORE_PEER_LOCALMSPID=Org2MSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org2.trch.com/users/Admin@org2.trch.com/msp" -e "CORE_PEER_ADDRESS=peer0.org2.trch.com:7051" cli peer chaincode install -n "$CC_NAME" -v "$CC_VERSION" -p "$CC_SRC_PATH" -l "$LANGUAGE"

# Chaincode Instantiate
docker exec -e "CORE_PEER_LOCALMSPID=Org2MSP" -e "CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org2.trch.com/users/Admin@org2.trch.com/msp" -e "CORE_PEER_ADDRESS=peer0.org2.trch.com:7051" cli peer chaincode instantiate -o orderer.trch.com:7050 -C trpwchannel -n "$CC_NAME" -l "$LANGUAGE" -v "$CC_VERSION" -c '{"Args":[""]}' -P "OR ('Org2MSP.member')"
