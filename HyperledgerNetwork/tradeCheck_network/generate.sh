...
configtxgen -profile OneOrgChannel -outputCreateChannelTx ./config/trpwchannel.tx -channelID $CHANNEL_NAME
if [ "$?" -ne 0 ]; then
...
configtxgen -profile TwoOrgChannel -outputCreateChannelTx ./config/trcrchannel.tx -channelID $CHANNEL_NAME2
if [ "$?" -ne 0 ]; then
  echo "Failed to generate channel configuration transaction..."
  exit 1
fi
...
configtxgen -profile TwoOrgChannel -outputAnchorPeersUpdate ./config/Org3MSPanchors.tx -channelID $CHANNEL_NAME2 -asOrg Org3MSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for Org2MSP..."
  exit 1
fi
...
configtxgen -profile TwoOrgChannel -outputAnchorPeersUpdate ./config/Org4MSPanchors.tx -channelID $CHANNEL_NAME2 -asOrg Org4MSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for Org2MSP..."
  exit 1
fi