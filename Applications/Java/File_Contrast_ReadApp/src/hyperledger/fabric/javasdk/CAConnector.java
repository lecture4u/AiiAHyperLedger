...
public class CAConnector {
	...
	private AppUser getAdmin(HFCAClient caClient, String adminID, String adminPW) throws Exception {
		AppUser admin = tryDeserialize("admin2");
		if (admin == null) {
			Enrollment adminEnrollment = caClient.enroll(adminID, adminPW);
			admin = new AppUser("admin2", "org2", "Org2MSP", adminEnrollment);
			serialize(admin);
		}
		return admin;
	}
	private AppUser getUser(HFCAClient caClient, AppUser registrar, String userId) throws Exception {
		...
		if (appUser == null) {
			RegistrationRequest rr = new RegistrationRequest(userId, "org2");
			String enrollmentSecret = caClient.register(rr, registrar);
			Enrollment enrollment = caClient.enroll(userId, enrollmentSecret);
			appUser = new AppUser(userId, "org2", "Org2MSP", enrollment);
			serialize(appUser);
		}
		return appUser;
	}
